...
export default apiInitializer("0.11.1", (api) => {
  ...
  function cardLooksLikeOurPlugin(card) {
    ...
    return Boolean(
      card.querySelector?.('a[href*="Discourse-Disify-Email-Protection-Plugin"]')
    );
  }
  ...
  function rewriteSettingsLinkInCard(card) {
    ...
    for (const anchor of anchors) {
      if (anchor.dataset.disifyEmailProtectionSettingsFixed === "1") {
        continue;
      }

      anchor.setAttribute("href", FIXED_SETTINGS_URL);
      anchor.dataset.disifyEmailProtectionSettingsFixed = "1";
    }
  }
